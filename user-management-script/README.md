...
You can use this script directly from this repository or create your own copy.
...
1. Clone the Bash Automation Scripts Collection and navigate to this module:
   ...
   cd bash-scripts/user-management-script
   ```

2. Make the script executable:

   ```bash
   chmod +x create_user_ssh_pub.sh
   ```

3. Execute the script with elevated privileges:

   ```bash
   sudo ./create_user_ssh_pub.sh
   ```

4. Follow the interactive prompts to provide the username, password, and SSH public key path (for example, `/home/dmitry/.ssh/id_rsa.pub`).

### Manual setup (as originally documented)

To manually create and run the script (as in the original standalone repository):
...
3. **Create script file and add the script contents:**

   ```bash
   touch create_user_ssh_pub.sh
   ```

   Then paste the script contents from the original repository into the file:

   - https://github.com/DimitryZH/bash-user-management-script/blob/main/create_user_ssh_pub.sh

4. **Execute the script:**

   ```bash
   sudo bash create_user_ssh_pub.sh
   ```
...
The "Bash User Management Script" project aims to simplify and automate user management tasks on Unix-like systems, enhancing administrative efficiency and consistency. This project offers a reliable and user-friendly solution for system administrators and DevOps professionals.

## Repository Context

This module is part of the Bash Automation Scripts Collection. For an overview of other modules and contribution guidelines, see the root [`README.md`](../README.md).
